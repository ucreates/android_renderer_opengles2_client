...
import com.ucreates.client.behaviour.SphereBehaviour1;
...
public class SpehreActivity1 extends AppCompatActivity implements GLSurfaceView.Renderer {
    private GLES2Renderer renderer;
    private ArrayList<SphereBehaviour1> behaviours;
    @Override
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        this.behaviours = new ArrayList<SphereBehaviour1>();
        for (int i = 0; i < 1; i++) {
            this.behaviours.add(new SphereBehaviour1(context));
        }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        for (int i = 0; i < this.behaviours.size(); i++) {
            SphereBehaviour1 behaviour = this.behaviours.get(i);
