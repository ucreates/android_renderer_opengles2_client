...
import com.ucreates.client.behaviour.CircleBehaviour1;
...
public class CircleActivity1 extends AppCompatActivity implements GLSurfaceView.Renderer {
    private GLES2Renderer renderer;
    private ArrayList<CircleBehaviour1> behaviours;
    @Override
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        this.behaviours = new ArrayList<CircleBehaviour1>();
        for (int i = 0; i < 1; i++) {
            this.behaviours.add(new CircleBehaviour1(context));
        }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        for (int i = 0; i < this.behaviours.size(); i++) {
            CircleBehaviour1 behaviour = this.behaviours.get(i);
